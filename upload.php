...
***********************************************************/
	if (!isset($_POST['x']) && !isset($_FILES['image']['name']) ){
		//Delete users temp image
			$temppath = 'img/uploads/avatar/'.$profile_id.'_temp.jpeg';
...
***********************************************************/
	...
	//Set Upload directory
		$uploaddir = $_SERVER['DOCUMENT_ROOT'].'/img/uploads/avatar';
	//Set File name
	...
	//Move the file to correct location
		...
		if (!$move) {
			...
			$imgSrc= "img/uploads/avatar/".$file_name; // the image to display in crop area
...
if (isset($_POST['x'])){
	...
	//the file type posted
		$type = $_POST['type'];
	//the image src
		$src = 'img/uploads/avatar/'.$_POST['src'];
	...
	if($type == 'jpg' || $type == 'jpeg' || $type == 'JPG' || $type == 'JPEG'){
		...
		//save the new cropped version
			imagejpeg($dst_r, "img/uploads/avatar/".$finalname."n.jpeg", 90);
		...
		$result_path ="img/uploads/avatar/".$finalname."n.jpeg";

		//Insert image into database
		$insert_pic_query = mysqli_query($connect_social, "UPDATE users SET avatar='$result_path' WHERE username='$userLoggedIn'");
